...
@valve_bp.route("/valves/list", methods=["GET"])
def list_valves():
    """Get a list of all valves."""
    response = get_all_valves()
    return jsonify(response)

@valve_bp.route("/valves/add", methods=["POST"])
def add_new_valve():
    """Add a new valve."""
    data = request.json
...
@valve_bp.route("/valves/<int:valve_id>/delete", methods=["DELETE"])
def delete_existing_valve(valve_id):
    """Delete a valve."""
    response = delete_valve(valve_id)
    return jsonify(response)

@valve_bp.route("/valves/<int:valve_id>/update", methods=["PUT"])
...
@valve_bp.route("/valves/<int:valve_id>/status", methods=["GET"])
